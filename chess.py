...
def makeMove(moveIn, board, real):
    # setting values for upcoming code based on whether the peice being moved is
    # white or black.
    if board[moveIn[1]][moveIn[0]][1] == "W":
        ...
        epPos = "4"
        pPos = "1"
    # Removes the peice being attacked in an en pasant
...
# in calculating the moves that peices can make.
...
# Checks if a position on the board is a peice or not.
def validPiece(col, board, Y, X):
    for piece in ("K", "P", "B", "Q", "R"):
        if board[Y][X] == (" " + col + piece) or board[Y][X] == (col + "Kn"):
            return True

# The next four functions are used to determine the positions that peices are attacking. This
# information will then be used to prevent the king from moving into an attacked position.

# Used for directional peices (rook, bishop and queen)
...
# Returns the opposite colour to the one provided in the colour format used for peices on the board.
def oppositeCol(col):
    if col == "white":
        return "B"
    elif col == "black":
...
# Returns the same colour to the one provided in the colour format used for peices on the board.
def findCol(col):
    if col == "white":
        return "W"
    elif col == "black":
...
def posCheck(start, end, trnCol, board):
    ...
    if kingSafetyCheck(board, col, oppCol, start, end) == "x":
        ...
        peice = board[s1][s0]
        # Note how the following checks reuse the functions used to determine where these peices could attack.
        # Knight
        if peice == (col + "Kn"):
